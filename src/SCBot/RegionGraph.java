...
public class RegionGraph {
	...
	public ArrayList<Region> getRegionsWithStatus(RegionStatus status) {
		...
		for(Node n : sorted) {
			if(n.status == status) regions.add(n.region);
		}
		return regions;
	}
	...
	public RegionStatus recordUnitDiscovery(Unit unit) {
		...
		if(unit.getPlayer() == game.enemy() && unit.getType().isBuilding() && !node.enemyBuildings.containsKey(unit.getID())) {
			...
			switch(node.status) {
			case ALLIED: node.status = RegionStatus.THREATENED;
				break;
			case ENEMY:
			case THREATENED:
			case NEUTRAL: node.status = RegionStatus.ENEMY;
			}
		}
		if(unit.getPlayer() == game.self() && unit.getType().isBuilding()) {
			node.alliedBuildings++;
			switch(node.status) {
			case ENEMY: node.status = RegionStatus.THREATENED;
				break;
			case ALLIED:
			case THREATENED:
			case NEUTRAL: node.status = RegionStatus.ALLIED;	
			}
		}
		return node.status;
	}
	...
	public RegionStatus recordUnitDestruction(Unit unit) {
		...
		if(unit.getPlayer() == game.self()) {
			if(node.status == RegionStatus.ALLIED) node.status = RegionStatus.THREATENED;
			if(unit.getType().isBuilding()) {
				if(--node.alliedBuildings < 1) node.status = RegionStatus.NEUTRAL;
			}
		}
		return node.status;
	}
